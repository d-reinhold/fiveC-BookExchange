...
$(document).ready ->
  ...
  $("input[name='search_type']").change ->
    if $("input#search_type_books").is(':checked')
      $("#searchbar-books").css "display", "block"
      $("#searchbar-courses").css "display", "none"
      $(".checkboxes").css "display", "none"
      $("#search_books").val($("#search_courses").val())
      $("#search_courses").val("")
    else
      $("#searchbar-courses").css "display", "block"
      $("#searchbar-books").css "display", "none"
      $(".checkboxes").css "display", "block"
      $("#search_courses").val($("#search_books").val())
      $("#search_books").val("")
  ...
  $("body").delegate "button#sell-form-final-signup-button", "click", (event) ->
    if $("button#sell-form-final-signup-button").hasClass("enabled")
      ...
      $("#new_user input#user_listings_attributes_0_book_id").val($("#check-email-form input#book_id_checker").val())
  ...
  $("body").delegate "button#sell-form-final-signin-button", "click", (event) ->
    if $("button#sell-form-final-signin-button").hasClass("enabled")
      ...
      $("#new_listing input#listing_book_id").val($("#check-email-form input#book_id_checker").val())
      ...
      $("#new_listing select#listing_condition").prop("selectedIndex", $("#check-email-form #condition_checker").prop("selectedIndex"))
...
check_form = () ->
  ...
  if PRICE_DOLLARS_READY
    $("button#sell-form-signed-in-final-button").removeClass("disabled") 
    $("button#sell-form-signed-in-final-button").addClass("enabled")
    if EMAIL_READY
